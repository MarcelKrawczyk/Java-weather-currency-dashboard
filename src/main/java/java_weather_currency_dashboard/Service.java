...
public class Service {
    ...
    public Service(String countryName) {
        ...
        this.countryCode = findCountryCode(countryName);
        this.currencyCode = findCurrencyCode(countryName);
    }

    private String findCountryCode(String countryName) {
    ...
    private String findCurrencyCode(String countryName) {
        for (Locale locale : Locale.getAvailableLocales()) {
            try {
                if (locale.getDisplayCountry(Locale.ENGLISH).equalsIgnoreCase(countryName)) {
    ...
    public Double getNBPRate() {
        ...
        for (String table : new String[]{"A", "B"}) {
            try {
                URI uri = new URI("http://api.nbp.pl/api/exchangerates/rates/" + table + "/" + currencyCode + "/?format=json");
                String json = getResponse(uri);
